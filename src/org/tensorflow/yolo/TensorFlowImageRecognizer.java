...
public class TensorFlowImageRecognizer {
    ...
    public static TensorFlowImageRecognizer create(AssetManager assetManager) {
        ...
        recognizer.inferenceInterface = new TensorFlowInferenceInterface(assetManager,
                "file:///android_asset/" + Config.MODEL_FILE);
        recognizer.outputSize = YOLOClassifier.getInstance()
                .getOutputSizeByShape(recognizer.inferenceInterface.graphOperation(Config.OUTPUT_NAME));
        return recognizer;
    }
    ...
    private float[] runTensorFlow(final Bitmap bitmap) {
        ...
        inferenceInterface.feed(Config.INPUT_NAME, processBitmap(bitmap), 1, Config.INPUT_SIZE, Config.INPUT_SIZE, 3);

        // Run the inference call.
        inferenceInterface.run(new String[]{Config.OUTPUT_NAME});

        // Copy the output Tensor back into the output array.
        inferenceInterface.fetch(Config.OUTPUT_NAME, tfOutput);
    ...
    private float[] processBitmap(final Bitmap bitmap) {
        int[] intValues = new int[Config.INPUT_SIZE * Config.INPUT_SIZE];
        float[] floatValues = new float[Config.INPUT_SIZE * Config.INPUT_SIZE * 3];

        bitmap.getPixels(intValues, 0, bitmap.getWidth(), 0, 0, bitmap.getWidth(), bitmap.getHeight());
        for (int i = 0; i < intValues.length; ++i) {
            final int val = intValues[i];
            floatValues[i * 3 + 0] = (((val >> 16) & 0xFF) - Config.IMAGE_MEAN) / Config.IMAGE_STD;
            floatValues[i * 3 + 1] = (((val >> 8) & 0xFF) - Config.IMAGE_MEAN) / Config.IMAGE_STD;
            floatValues[i * 3 + 2] = ((val & 0xFF) - Config.IMAGE_MEAN) / Config.IMAGE_STD;
        }
        return floatValues;
    }
}
